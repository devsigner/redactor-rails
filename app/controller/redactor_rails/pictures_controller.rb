class RedactorRails::PicturesController < ApplicationController
  before_filter :authenticate_admin_user! if RedactorRails.picture_model.new.respond_to?(:user_id)

  def index
    @pictures = RedactorRails.picture_model.where(
        RedactorRails.picture_model.new.respond_to?(:user_id) ? { user_id: current_admin_user.id } : { })
